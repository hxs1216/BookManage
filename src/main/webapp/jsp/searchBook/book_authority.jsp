...
<center>
    ...
    <form method="post">
        ...
        <div id="main">
            <div class="tablebox">
                <h1 class="authority" style="margin-top: 188px;">${searchKey}<span class="small">的作品</span></h1>
                <table class="index_table">
                    ...
                    <tr bgcolor="${(status.index)%2==0?'#eee':'#ffffff'}">
                        <td><input  id="authority_${book.id}" type="checkbox" onclick="SetArticleId(this,'authority_${book.id}');"/></td>
...
<script type="text/javascript">
    ...
    function addShopping(){
        ...
        check.splice(check.length-1,1);
        for(var i=0;i<check.length;i++){
            if(check[i].indexOf("authority") != -1){
    ...
    function allCheck(obj){
        ...
        if(check){
            $("#authority_"+${book.id})[0].checked = true;
            AddCookie('authority_'+${book.id});
        }else{
            $("#authority_"+${book.id})[0].checked = false;
            RemoveCookie('authority_'+${book.id});
        }
        </c:forEach>
    }
</script>
