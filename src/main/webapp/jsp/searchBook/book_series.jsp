    ...
    <table class="index_table">
        ...
        <c:forEach items="${books}" var="book" varStatus="status">
            <tr bgcolor="${(status.index)%2==0?'#eee':'#ffffff'}">
                <td><input id="series_${book.id}"  id="${book.id}" type="checkbox" onclick="SetArticleId(this,'series_${book.id}');"/></td>
...
<script type="text/javascript">
    ...
    function allCheck(obj){
        ...
        if(check){
            $("#series_"+${book.id})[0].checked = true;
            AddCookie('series_'+${book.id});
        }else{
            $("#series_"+${book.id})[0].checked = false;
            RemoveCookie('series_'+${book.id});
        }
        </c:forEach>
    }
</script>
